...
<? 
	OpenDb();		
	$sql = "SELECT k.replid, s.nis, s.nama, k.komentar FROM siswa s, komennap k, infonap i WHERE s.idkelas = $kelas AND k.nis = s.nis AND k.idinfo = i.replid AND i.idkelas = $kelas AND i.idpelajaran = $pelajaran AND i.idsemester= $semester ORDER BY $urut $urutan";
	//$sql = "SELECT k.replid, s.nis, s.nama, k.komentar FROM siswa s LEFT JOIN (komennap k INNER JOIN infonap i ON k.idinfo = i.replid AND i.idpelajaran = $pelajaran AND i.idkelas = $kelas AND i.idsemester = $semester) ON k.nis = s.nis WHERE s.idkelas = $kelas ORDER BY s.nama";
	//echo $sql;
	$result = QueryDb($sql);		
	$cnt = 1;
...
<tr>
	...
	<table border="1" width="100%" id="table" class="tab" bordercolor="#000000" >
		<tr>		
			...
			<td height="30" onMouseOver="background='../style/formbg2agreen.gif';height=30;" onMouseOut="background='../style/formbg2.gif';height=30;" background="../style/formbg2.gif" style="cursor:pointer;" onClick="change_urut('nama','<?=$urutan?>')"><div align="center" class="style1"><strong><span class="style3">Siswa</span></strong></div></td>
		</tr>
        ...
        <tr>        			
            ...
            <input type="hidden" name="replid<?=$cnt?>" value="<?=$row['replid']?>">
			<input type="hidden" name="nis<?=$cnt?>" value="<?=$row['nis']?>">
  			<td height="25" onclick="tampil('<?=$row[nis]?>')" style="cursor:pointer" title="Klik untuk menampilkan komentar rapor <?=$row['nama']?>"><?=$row['nis']?><br /><?=$row['nama']?></td>
        </tr>
 	<?		$cnt++;
		} 
		CloseDb();	?>
