...
HitungRataKelas($_REQUEST['kelas'],$_REQUEST['semester'],$_REQUEST['idaturan']);
?>
...
function tambah(){
	newWindow('nilai_pelajaran_add.php?idaturan=<?=$idaturan?>&semester=<?=$semester?>&kelas=<?=$kelas?>','TambahNilai','580','600','resizable=1,scrollbars=1,status=0,toolbar=0');
}
...
<? 
	$sql_cek_ujian = "SELECT u.replid, u.tanggal, u.deskripsi, u.idrpp FROM jbsakad.ujian u WHERE u.idaturan=$idaturan AND u.idkelas=$kelas AND u.idsemester=$semester ORDER by u.tanggal ASC";
...
<? 			}
	...
	<?
        ...
        while ($row_siswa=@mysql_fetch_array($result_siswa)){
            $nilai = 0;
			HitungRataSiswa($_REQUEST['kelas'],$_REQUEST['semester'],$_REQUEST['idaturan'],$row_siswa['nis']);
		...
		<?  } ?>
    		<td align="center"><?=GetRataSiswa($_REQUEST['kelas'],$_REQUEST['semester'],$_REQUEST['idaturan'],$row_siswa['nis'])?><?//=round($nilai/count($idujian),2)?></td>
    		<td align="center">
	<?					
  ...
  <?
    ...
    <tr>
  	...
  	<tr>
        ...
        <tr>
    		<td width="55%">  
           	<input <?=$dis_btn?> type="button" name="hitung" id="hitung" value="Hitung dan Simpan Nilai Akhir <?=$namajenis?>" class="but" onClick="return validate(); document.getElementById('tampil_nilai_pelajaran').submit();" />
     	...
     	<? 
			...
			$sql_cek_ujian="SELECT *, u.replid as replid FROM jbsakad.ujian u WHERE u.idaturan=$idaturan AND u.idkelas=$kelas AND u.idsemester=$semester ORDER by u.tanggal ASC";
			$result_cek_ujian=QueryDb($sql_cek_ujian);
		...
		?>
            ...
            <? 	if (mysql_num_rows($result_get_bobotnya) > 0) { ?>
				<input <?=$dis?> type="checkbox" name="<?='jenisujian'.$ibobot ?>" id="<?='jenisujian'.$ibobot ?>" value="1" checked  onKeyPress="return focusNext('bobot<?=$ibobot?>',event)">				
            	<input type="hidden" name="<?='replid'.$ibobot?>" id = "<?='replid'.$ibobot?>" value="<?=$nilai_bobotnya['replid']?>">
			<?  } else { ?>
				<input <?=$dis?> type="checkbox" name="<?='jenisujian'.$ibobot ?>" id="<?='jenisujian'.$ibobot ?>" value="1"  onKeyPress="return focusNext('bobot<?=$ibobot?>',event)"> 
            <? } ?>
               
               	</td>
                <td> <?=$namajenis."-".$ibobot." (".format_tgl($row_cek_ujian['tanggal']).")"; ?>
                	<br>RPP: <?=$namarpp?>
