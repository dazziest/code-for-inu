		...
		<? for ($i=0;$i<count($semester);$i++){ ?>
			<div class="TabbedPanelsContent">
				...
				<div id="TabbedPanelsA<?=$i?>" class="TabbedPanels" style="z-index:1">
				  <ul class="TabbedPanelsTabGroup">
					<? 
					$sql5 = "SELECT * FROM dasarpenilaian";
					$res5 = QueryDb($sql5);
					?>
					<? while ($row5 = @mysql_fetch_array($res5)){ ?>
					<li class="TabbedPanelsTab" tabindex="0"><?=$row5['dasarpenilaian']?></li>
					<? } ?>
					...
					<div class="TabbedPanelsContent">
						...
						while ($row = @mysql_fetch_row($res)){
							...
							while ($row2 = @mysql_fetch_row($res2)){
								$sql3 = "SELECT nilaiRK FROM ratauk WHERE idkelas='$kls' AND idsemester='".$semester[$i][0]."' AND idujian='$row2[3]'";
								$res3 = QueryDb($sql3);
								$row3 = @mysql_fetch_row($res3);
								$ratauk = $row[0];
								$prosen = round(($row2[2]/100)-($row3[0]/100),2);
								if ($prosen>0)
									$prosen = "+".$prosen;	
								$numnilai += $row2[2];
