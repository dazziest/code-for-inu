...
$sql_get_nhb="SELECT DISTINCT k.kelas, t.tingkat, t.replid AS idtingkat, t.departemen, p.nama, p.replid AS idpelajaran, ta.tahunajaran, s.semester, j.jenisujian, j.replid AS idjenis FROM jbsakad.aturannhb a, kelas k, tahunajaran ta, tingkat t, pelajaran p, semester s, jenisujian j WHERE a.replid=$idaturan AND k.idtingkat = t.replid AND k.replid = $kelas AND s.replid = $semester AND p.replid = a.idpelajaran AND j.replid = a.idjenisujian AND ta.replid = k.idtahunajaran";
...
<html>
<head>
<title>AKADEMIK [Tambah Data Nilai Pelajaran]</title>
...
?>
    <tr height="25">
        ...
        <td align="center">
            <input type="text" name="nilaiujian[<?=$row_siswa['nis']?>][0]" id="nilaiujian<?=$i?>" size="4" maxlength="3" onKeyPress="return focusNext('keterangan<?=$i?>', event);" ></td>
        <td align="center">
