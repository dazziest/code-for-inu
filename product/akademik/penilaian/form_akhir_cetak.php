...
OpenDb();
$sql = "SELECT k.kelas AS namakelas, s.semester AS namasemester, a.tahunajaran, a.departemen, l.nama, t.tingkat, j.jenisujian, p.nama AS guru, n.dasarpenilaian,s.departemen as dep FROM kelas k, semester s, tahunajaran a, pelajaran l, tingkat t, aturannhb n, jenisujian j, jbssdm.pegawai p WHERE k.replid = $kelas AND s.replid = $semester AND  k.idtahunajaran = a.replid AND l.replid = n.idpelajaran AND t.replid = k.idtingkat AND n.replid = $idaturan AND n.idjenisujian = j.replid AND p.nip = '$nip'";
//echo $sql;
$result = QueryDb($sql);
CloseDb();
...
<title>Akademik [Cetak Form Pengisian Nilai Akhir Siswa]</title>
</head>
...
<br />
    ...
    <?
	 OpenDb();
    //$sql_nhb_PK = "SELECT u.tanggal as tanggal, u.replid as idujian, j.jenisujian as jenisujian FROM jbsakad.ujian u, jbsakad.aturannhb a, jbsakad.jenisujian j WHERE u.idaturan='$idaturan' ".
    //			  " AND u.idaturan=a.replid AND a.idjenisujian=j.replid";
                  
    $sql_nhb_PK = "SELECT * FROM jbsakad.ujian WHERE idaturan=$idaturan AND idkelas=$kelas AND idsemester=$semester ORDER by tanggal ASC";
    
    $result_nhb_PK = QueryDb($sql_nhb_PK);
    $cntujian=1;
    while ($row_nhb_PK = @mysql_fetch_array($result_nhb_PK)){
        $idujian[$cntujian] = $row_nhb_PK['replid'];
		$tgl = explode("-",$row_nhb_PK['tanggal']);
		
    ?>
        <td class="headerlong">
          <?=$row['jenisujian']."&nbsp;".$cntujian."<br>".$tgl[2]."/".$tgl[1]."/".substr($tgl[0],2)?>
        </td>
    <?
        $cntujian++;
    }
    ...
    <?
    //$sql_get_nis="SELECT n.nis,s.nama,s.aktif FROM jbsakad.nilaiujian n,jbsakad.ujian u, jbsakad.siswa s WHERE n.nis=s.nis AND u.idaturan='$idaturan' AND s.idkelas='$kelas' AND n.idujian = u.replid GROUP BY n.nis ORDER BY s.nama";
   	// echo $sql_get_nis;
