...
if (isset($_REQUEST['hapus'])) {
	...
	$sql = "SELECT k.replid as replid, s.nis, s.nama, k.komentar FROM siswa s, komennap k, infonap i WHERE s.idkelas = $kelas AND k.nis = s.nis AND k.idinfo = i.replid AND i.idkelas = $kelas AND i.idpelajaran = $pelajaran AND i.idsemester= $semester ORDER BY $urut $urutan";
	$result = QueryDb($sql);		
	$cnt = 1;
...
<table width="100%" border="1" cellspacing="0" class="tab" id="table">
  <tr>		
	...
	<th height="30" onMouseOver="background='../style/formbg2agreen.gif';height=30;" onMouseOut="background='../style/formbg2.gif';height=30;" background="../style/formbg2.gif" style="cursor:pointer;" onClick="change_urut('nis','<?=$urutan?>');"><div align="center" class="style1"><strong><span class="style2">NIS&nbsp;</span>&nbsp;
		...
		<tr>        			
			<td height="25" align="center"><?=$cnt?><input type="hidden" name="replid<?=$cnt?>" value="<?=$row[replid]?>"></td>
			
			<td height="25" align="center"><div align="left"><?=$row['nis']?></div></td>
            <input type="hidden" name="nis<?=$cnt?>" value="<?=$row['nis']?>">
  			<td height="25"><div align="left"><?=$row['nama']?></div></td>
            <td height="25"><div align="left">
              <?
			 $sql_get_comment="SELECT k.komentar,k.replid FROM jbsakad.komennap k, jbsakad.infonap i WHERE k.nis='$row[nis]' AND i.replid=k.idinfo AND i.idpelajaran='$pelajaran' AND i.idsemester='$semester' AND i.idkelas='$kelas'";
$result_get_comment=QueryDb($sql_get_comment);
$row_get_comment=@mysql_fetch_row($result_get_comment);
			  $ada_komentar=$row_get_comment[0];
              if ($ada_komentar<>""){
			  echo $row_get_comment[0];
			  } else {
			...
			if ($ada_komentar<>""){
			?>
              <img src="../images/ico/ubah.png" border="0" onClick="ubah('<?=$row_get_comment[1]?>','1')" style="cursor:pointer" title="Ubah Komentar Siswa Ini"/>
			<?
			  } else {
				?>
			<img src="../images/ico/tambah.png" border="0" onClick="ubah('<?=$row_get_comment[1]?>','0')" style="cursor:pointer" title="Input Komentar Siswa Ini"/>
			<?
			}
			//  }
			?>
