...
<title>Akademik [Cetak Form Nilai Akhir]</title>
...
<body topmargin="0" leftmargin="0" marginheight="0" marginwidth="0" style="background-color:#dcdfc4" onLoad="document.getElementById('<?=$fokus?>').focus();">
...
<tr height="100">
    ...
    <tr>
    	...
    	<td><select name="aspek" id="aspek" onChange="change_aspek()" style="width:150px" onKeyPress="return focusNext('aturan', event)">
			<? 
			$sql_daspen="SELECT DISTINCT(dasarpenilaian) as dasarpenilaian FROM jbsakad.aturannhb WHERE idtingkat=$tingkat AND idpelajaran=$pelajaran AND aktif=1 AND nipguru='$nip' ";
			...
			?>	
				<option value="<?=$row_daspen['dasarpenilaian']?>" <?=StringIsSelected($row_daspen['dasarpenilaian'], $aspek) ?>><?=$row_daspen['dasarpenilaian']?></option>
			<? } ?>
  		    </select>
  		</td>
  	</tr>
	...
	?>
  	<tr>
    	...
    	<td>
			...
			<?
			$sql_jenispengujian="SELECT a.replid, j.jenisujian FROM jbsakad.aturannhb a, jbsakad.jenisujian j WHERE a.idtingkat=$tingkat AND a.idpelajaran=$pelajaran AND a.aktif=1 AND a.dasarpenilaian='$aspek' AND nipguru='$nip' AND j.replid = a.idjenisujian";
	...
	<? if ($num_jenispengujian!=0){ 
			OpenDb();
			$sql = "SELECT * FROM ujian WHERE idkelas = $kelas AND idaturan = $aturan AND idsemester = $semester";
			//echo $sql;
			$result = QueryDb($sql);
