...
OpenDb();
$sql = "SELECT s.replid, s.semester, p.nama FROM semester s, pelajaran p WHERE s.departemen = '$departemen' AND p.replid = $pelajaran AND p.departemen = '$departemen' AND s.semester='Genap'"; 
$result = QueryDb($sql);
	...
	?>
   	<tr>
        ...
        <tr>        			
			...
			<td height="25"><?=format_tgl($row1[0])?></td>
            <td width="8" height="25"><?=$row1[1]?></td>
