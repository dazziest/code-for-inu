...
OpenDb();
$sql = "SELECT j.departemen, j.nama, p.nip, p.nama FROM guru g, jbssdm.pegawai p, pelajaran j WHERE g.nip=p.nip AND g.idpelajaran = j.replid AND j.replid = $id_pelajaran AND g.nip = '$nip_guru'"; 
...
function tambah(tingkat) {
	...
	var nip = document.getElementById('nip_guru').value;
	newWindow('perhitungan_rapor_add.php?id_tingkat='+tingkat+'&id_pelajaran='+id+'&nip_guru='+nip, 'TambahAturanPerhitunganNilaiRapor','360','515','resizable=1,scrollbars=1,status=0,toolbar=0')
}

function edit(tingkat,aspek) {
	var id = document.getElementById('id_pelajaran').value;
	var nip = document.getElementById('nip_guru').value;
	newWindow('perhitungan_rapor_edit.php?id_tingkat='+tingkat+'&id_pelajaran='+id+'&nip_guru='+nip+"&aspek="+aspek, 'UbahAturanPerhitunganNilaiRapor','360','515','resizable=1,scrollbars=1,status=0,toolbar=0')
}
...
<tr>
  ...
  <?
    ...
    while ($row_tkt = @mysql_fetch_array($result)) {
		++$cnt;
		OpenDb();
		$query_at = "SELECT dasarpenilaian FROM aturannhb a, tingkat t ".
