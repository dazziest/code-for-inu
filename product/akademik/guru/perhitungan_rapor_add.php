...
<title>AKADEMIK [Tambah Aturan Perhitungan Nilai Rapor]</title>
    ...
    <!-- CONTENT GOES HERE //--->
		...
		<tr>
			...
			<td><select name="aspek" id="aspek" onKeyPress="focusNext('cek1',event)">
          	<?
          	OpenDb();
			$sql = "SELECT dasarpenilaian FROM dasarpenilaian WHERE dasarpenilaian NOT IN (SELECT a.dasarpenilaian FROM guru g, pelajaran j, dasarpenilaian d, tingkat t, aturannhb a WHERE a.nipguru=g.nip AND a.idpelajaran = j.replid AND a.dasarpenilaian = d.dasarpenilaian AND a.idtingkat = t.replid AND a.idpelajaran = $id_pelajaran AND a.nipguru = '$nip_guru' AND a.idtingkat = $id_tingkat )";    
			$result = QueryDb($sql);	
			CloseDb();
