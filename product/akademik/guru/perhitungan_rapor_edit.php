...
OpenDb();
$sql = "SELECT j.departemen, j.nama, p.nip, p.nama, t.tingkat FROM guru g, jbssdm.pegawai p, pelajaran j, tingkat t WHERE g.nip=p.nip AND g.idpelajaran = j.replid AND t.departemen = j.departemen AND t.replid = $id_tingkat AND j.replid = $id_pelajaran AND g.nip = '$nip_guru'"; 
$result = QueryDb($sql);
CloseDb();
...
if (isset($_REQUEST['Simpan'])) {
	...
	for ($i=1;$i<=$jum;$i++) {
		...
		if ($jenis && $bobot) {
			if ($id) {
				$sql = "UPDATE aturannhb SET bobot=$bobot WHERE replid = $id";				
				$result = QueryDb($sql);
				
			} else {
				$sql = "INSERT INTO aturannhb SET nipguru='$nip_guru',idtingkat=$id_tingkat,idpelajaran=$id_pelajaran,dasarpenilaian='$aspek',idjenisujian=$jenis,bobot=$bobot";				
				$result = QueryDb($sql);
			}	
		}
	}
...
<title>AKADEMIK [Ubah Aturan Perhitungan Nilai Rapor]</title>
...
<!-- TABLE CONTENT -->
<tr>
	<td width="120"><strong>Sekolah</strong></td>
	<td>
    	<input type="text" name="departemen" id="departemen" size="30" maxlength="50" readonly value="<?=$departemen ?>" class="disabled" />
...
<fieldset><legend><b>Bobot Penilaian</b></legend>
	...
	<table border="0" width="100%"  id="table" class="tab">
		...
		while ($row = @mysql_fetch_array($result)) {
			...
			$sql1 = "SELECT a.bobot, j.jenisujian, a.replid FROM aturannhb a, jenisujian j WHERE a.idpelajaran = $id_pelajaran AND a.nipguru = '$nip_guru' AND a.idtingkat = $id_tingkat AND a.dasarpenilaian = '$aspek' AND a.idjenisujian = $row[replid] AND a.idjenisujian = j.replid"; 
		...
		<tr>
        	...
        	<? 	if ($row1[1]) { 
				?>
			<input type="checkbox" name="<?='cek'.$i ?>" id="<?='cek'.$i ?>" value="1" checked  onKeyPress="focusNext('bobot<?=$i?>',event)">
