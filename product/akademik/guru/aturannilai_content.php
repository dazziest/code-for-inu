...
OpenDb();
$sql = "SELECT j.departemen, j.nama, p.nip, p.nama FROM guru g, jbssdm.pegawai p, pelajaran j WHERE g.nip=p.nip AND g.idpelajaran = j.replid AND j.replid = $id AND g.nip = '$nip'"; 
...
<tr>
    ...
    <tr>
        ...
        <td width="10%"><strong>Sekolah</strong></td>
	...
	while ($row = @mysql_fetch_array($result)) {
		++$i;
		OpenDb();
		$sql1 = "SELECT dasarpenilaian FROM aturangrading g, tingkat t WHERE t.replid = g.idtingkat AND t.departemen = '$departemen' AND g.idpelajaran = $id AND g.idtingkat = $row[replid] AND g.nipguru = '$nip' GROUP BY g.dasarpenilaian";
		//echo $sql1;
    ...
    <br />
 	...
 	<?	if (@mysql_num_rows($result1) > 0){ 
		...
		<tr>        			
			...
			<td height="25"><?=$row1[0]?><input type="hidden" name="dasar" id="dasar" value="<?=$row1[0] ?>" /></td>
  			<td height="25">
			<? 
			OpenDb();
			$sql2 = "SELECT g.replid, grade, nmin, nmax FROM aturangrading g, tingkat t WHERE t.replid = g.idtingkat AND t.departemen = '$departemen' AND g.idpelajaran = $id AND g.idtingkat = $row[replid] AND g.dasarpenilaian = '$row1[0]' AND g.nipguru = '$nip' ORDER BY grade";
...
<tr>
	<td align="center" valign="middle" height="200">
    	<font size = "2" color ="red"><b>Tidak ditemukan adanya data.
        <br />Tambah tingkat kelas pada sekolah <?=$departemen?> di menu Referensi.
        </b></font>
	</td>
</tr>
</table>  
