...
<title>AKADEMIK [Tambah Aturan Penentuan Grading Nilai]</title>
...
<tr>
	...
	<td><input type="text" size="30" maxlength="50" class="disabled" readonly value="<?=$departemen ?>" />
...
<tr>
	...
	<td>
    	<select name="aspek" id="aspek" onKeyPress="focusNext('nmin1',event)">
          <?
          	OpenDb();
			$sql = "SELECT dasarpenilaian FROM dasarpenilaian WHERE dasarpenilaian NOT IN (SELECT dasarpenilaian FROM aturangrading g, tingkat t WHERE t.replid = g.idtingkat AND g.idpelajaran = $id AND g.idtingkat = $idtingkat AND g.nipguru = '$nip' GROUP BY g.dasarpenilaian) ";    
			$result = QueryDb($sql);	
			CloseDb();
