...
if (isset($_REQUEST['Simpan'])) {
	OpenDb();
	$sql = "SELECT * FROM departemen WHERE departemen = '$_REQUEST[departemen]' AND replid <> $replid";
	$result = QueryDb($sql);
	...
	if (mysql_num_rows($result) > 0) {
		...
		$sql = "UPDATE departemen SET departemen='$_REQUEST[departemen]',nipkepsek='$_REQUEST[nip]',urutan=$_REQUEST[urutan],keterangan='$_REQUEST[keterangan]' WHERE replid=$replid";
		$result = QueryDb($sql);
		CloseDb();
...
OpenDb();
$sql = "SELECT d.departemen,d.nipkepsek,p.nama,d.urutan,d.keterangan FROM departemen d, jbssdm.pegawai p WHERE d.nipkepsek = p.nip AND d.replid = '$replid'"; 
...
<title>Akademik [Ubah Sekolah]</title>
...
<tr>
	...
	<td>
    	<input type="text" name="departemen" id="departemen" size="30" maxlength="50" value="<?=$departemen ?>" onFocus="showhint('Nama sekolah tidak boleh lebih dari 50 karakter!', this, event, '120px');panggil('departemen')"  onKeyPress="return focusNext('nip', event)"/>
    </td>
</tr>
<tr>
...
<tr>
	...
	<td>
    	<input type="text" name="urutan" id="urutan" size="3" maxlength="5" value="<?=$urutan ?>" onFocus="showhint('Urutan penampilan sekolah', this, event, '120px');panggil('urutan')" onKeyPress="return focusNext('keterangan', event)"/>
    </td>
</tr>
<tr>
