...
if (isset($_REQUEST['Simpan'])) {
	...
	if (mysql_num_rows($result) > 0) {
		...
		BeginTrans();
		$success=0;		
		$sql="INSERT INTO jbsakad.siswa SET nis='$nis',nama='$row[nama]', panggilan='$row[panggilan]', tahunmasuk=$year, idangkatan=$angkatan, idkelas=$kelas, suku='$row[suku]', agama='$row[agama]', status='$row[status]', kondisi='$row[kondisi]', kelamin='$row[kelamin]', tmplahir='$row[tmplahir]', tgllahir='$row[tgllahir]', warga='$row[warga]', anakke=$row[anakke], jsaudara=$row[jsaudara], bahasa='$row[bahasa]', berat=$row[berat], tinggi=$row[tinggi], darah='$row[darah]', alamatsiswa='$row[alamatsiswa]', $kodepos_sql, telponsiswa='$row[telponsiswa]', hpsiswa='$row[hpsiswa]', emailsiswa='$row[emailsiswa]', kesehatan='$row[kesehatan]', $sekolah_sql, ketsekolah='$row[ketsekolah]', namaayah='$row[namaayah]', namaibu='$row[namaibu]', almayah=$row[almayah], almibu=$row[almibu], $pendidikanayah_sql, $pendidikanibu_sql, $pekerjaanayah_sql, $pekerjaanibu_sql, wali='$row[namawali]', penghasilanayah=$row[penghasilanayah], penghasilanibu=$row[penghasilanibu], alamatortu='$row[alamatortu]', telponortu='$row[telponortu]', hportu='$row[hportu]', emailayah='$row[emailayah]', emailibu='$row[emailibu]', alamatsurat='$row[alamatsurat]', keterangan='$row[keterangan]', frompsb=1, ketpsb='$keterangan', pinsiswa='$pinsiswa', pinortu='$pinortu', pinortuibu = '$pinortuibu'";
...
<?	
/*OpenDb();
$sql = "SELECT COUNT(*), k.kapasitas FROM siswa s, kelas k WHERE s.idkelas = $kelas AND s.idangkatan = $angkatan AND k.replid = s.idkelas AND k.idtahunajaran = $tahunajaran AND k.idtingkat = $tingkat GROUP BY s.idkelas";
$result = QueryDb($sql);
$jum = @mysql_num_rows($result);
$row = @mysql_fetch_row($result); 				
CloseDb();

if ($row[0] >= $row[1] && $jum <> 0) {  ?>
	<script>
		alert ('Kelas sudah melebihi kapasitas');
		window.close();
    </script>
   	
<? } else {*/ 
	OpenDb(); 
	$sql = "SELECT k.kelas, t.tahunajaran, a.angkatan, t.departemen FROM kelas k, tahunajaran t, angkatan a WHERE k.replid = $kelas AND t.replid = $tahunajaran AND k.idtahunajaran = t.replid AND a.replid = $angkatan AND a.departemen = '$departemen'";
...
<tr>
	...
	<td>
    	<input type="text" name="nis" id="nis" size="15" maxlength="15" value="<?=$nis ?>" onFocus="showhint('NIS tidak boleh lebih dari 15 karakter!', this, event, '120px')" onKeyPress="return focusNext('keterangan', event)"/>
    </td>
</tr>
<tr>
