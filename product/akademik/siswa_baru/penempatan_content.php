...
<? 
	...
	$sql_tot = "SELECT s.replid,s.nis,s.nama,s.frompsb FROM jbsakad.siswa s, jbsakad.kelas k, jbsakad.tahunajaran t WHERE s.idkelas = $kelas AND k.idtahunajaran = $tahunajaran AND k.idtingkat = $tingkat AND s.idangkatan = $angkatan AND s.idkelas = k.replid AND t.replid = k.idtahunajaran AND s.aktif=1";
	$result_tot = QueryDb($sql_tot);
	$total=ceil(mysql_num_rows($result_tot)/(int)$varbaris);
	$jumlah = mysql_num_rows($result_tot);
	$akhir = ceil($jumlah/5)*5;
	
	$sql = "SELECT s.replid,s.nis,s.nama,s.frompsb FROM jbsakad.siswa s, jbsakad.kelas k, jbsakad.tahunajaran t WHERE s.idkelas = $kelas AND k.idtahunajaran = $tahunajaran AND k.idtingkat = $tingkat AND s.idangkatan = $angkatan AND s.idkelas = k.replid AND t.replid = k.idtahunajaran AND s.aktif=1 ORDER BY $urut $urutan LIMIT ".(int)$page*(int)$varbaris.",$varbaris";
...
var nama = document.getElementById('nama').value;	
var warna =	document.getElementById('warna').value;	
	alert('masuk bawah'+kelompok);
	//alert ('dep '+departemen+' angkatan '+angkatan+' tahunajaran '+tahunajaran+' tingkat '+tingkat+' kelas '+kelas+' aktif '+aktif+' proses '+proses+' kelompok '+kelompok);	
	parent.menu.location.href = "penempatan_menu.php?departemen="+departemen+"&angkatan="+angkatan+"&tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&kelas="+kelas+"&proses="+proses+"&kelompok="+kelompok+"&cari="+cari+"&no="+no+"&nama="+nama+"&warna="+warna; 		
	parent.daftar.location.href = "penempatan_daftar.php?departemen="+departemen+"&angkatan="+angkatan+"&tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&kelas="+kelas+"&proses="+proses+"&kelompok="+kelompok+"&aktif="+aktif+"&cari="+cari+"&no="+no+"&nama="+nama; */
</script>
</body>
</html>