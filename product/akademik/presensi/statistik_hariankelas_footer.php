...
function cetak() {
	...
	newWindow('statistik_hariankelas_cetak.php?tglawal='+tglawal+'&tglakhir='+tglakhir+'&semester='+semester+'&tingkat='+tingkat, 'CetakStatistikKehadiranHarianKelas','800','650','resizable=1,scrollbars=1,status=0,toolbar=0');
}
</script>
...
<tr>
    ...
    <? 		
	OpenDb();
	$sql = "SELECT DISTINCT k.kelas, k.replid FROM presensiharian p, kelas k WHERE p.idkelas = k.replid AND k.idtingkat = $tingkat AND p.idsemester = $semester AND (((p.tanggal1 BETWEEN '$tglawal' AND '$tglakhir') OR (p.tanggal2 BETWEEN '$tglawal' AND '$tglakhir')) OR (('$tglawal' BETWEEN p.tanggal1 AND p.tanggal2) OR ('$tglakhir' BETWEEN p.tanggal1 AND p.tanggal2))) ORDER BY k.kelas, p.tanggal1 ";	
	//echo 'sql '.$sql;
	$result = QueryDb($sql);			 
	...
	?> 
        ...
        <tr height="25">        			
            ...
            <!--<td align="center"><img src="statistik_harianbatang.php?semester=<?=$semester?>&kelas=<?=$row[1]?>&bln1=<?=$bln1?>&bln2=<?=$bln2?>&th1=<?=$th1?>&th2=<?=$th2?>" />-->
