...
<tr>	
    ...
    <tr>
   	...
   	<tr>
        ...
        <td width="160">
          	<select name="bln1" id ="bln1" onchange="change_tgl1()" onKeyPress="return focusNext('th1',event)" onfocus="panggil('bln1')">
        <? 	for ($i=1;$i<=12;$i++) { ?>
		...
		<td>
        	<select name="bln2" id ="bln2" onchange="change_tgl2()" onKeyPress="return focusNext('th2',event)" onfocus="panggil('bln2')">
        <? 	for ($i=1;$i<=12;$i++) { ?>
        	<option value="<?=$i?>" <?=IntIsSelected($bln2, $i)?>><?=$bulan[$i]?></option>	
        <?	}	?>	
        	</select>
       	 	<select name="th2" id = "th2" onchange="change_tgl2()" onfocus="panggil('th2')" onKeyPress="return focusNext('tabel',event)" style="width:60px">
    ...
    <td width="*" rowspan="2" align="left" valign="middle"><a href="#" onclick="tampil()">
    	<img src="../images/view.png" height="48" width="48" border="0" name="tabel" id="tabel" onmouseover="showhint('Klik untuk menampilkan laporan presensi harian siswa!', this, event, '180px')"/></a></td>
