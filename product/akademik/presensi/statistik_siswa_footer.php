...
function cetak() {
	...
	newWindow('statistik_siswa_cetak.php?tglawal='+tglawal+'&tglakhir='+tglakhir+'&semester='+semester+'&kelas='+kelas+'&pelajaran='+pelajaran, 'CetakStatistikKehadiranSetiapSiswa','800','650','resizable=1,scrollbars=1,status=0,toolbar=0');
}
</script>
</head>
...
<tr>
	...
	?> 
        ...
        <tr height="25">        			
            ...
            <td align="center">
            <!--<img src="statistik_batang.php?semester=<?=$semester?>&kelas=<?=$kelas?>&pelajaran=<?=$pelajaran?>&bln1=<?=$bln1?>&bln2=<?=$bln2?>&th1=<?=$th1?>&th2=<?=$th2?>&nis=<?=$row[0]?>" />-->
            <img src="statistik_batang.php?semester=<?=$semester?>&kelas=<?=$kelas?>&pelajaran=<?=$pelajaran?>&tglakhir=<?=$tglakhir?>&tglawal=<?=$tglawal?>&nis=<?=$row[0]?>" />
            </td>
    	</tr>
 	<?		
		} 
...
<? 	} else { ?>
	...
	<tr>
    	...
    	<font size = "2" color ="red"><b>Tidak ditemukan adanya data. <br /><br />Tambah data presensi kelas di menu Presensi Harian atau <br />Presensi Pelajaran pada bagian Presensi.</b></font>
		</td>
	</tr>
	</table>
<?	} ?>
