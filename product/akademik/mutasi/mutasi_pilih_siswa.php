...
//$nama = $_REQUEST['nama'];
//$nis = $_REQUEST['nis'];
$sql = "SELECT s.nis, s.nama, k.kelas FROM jbsakad.siswa s,jbsakad.kelas k WHERE k.replid=s.idkelas AND k.replid='$kelas' ORDER BY s.nama"; 
$result = QueryDb($sql);
$cnt = 1;
...
if ($cnt%2==1)
	$bg="bgcolor='#ffffff'";	?>
<tr>
	<td align="center" height="25" <?=$bg?> onClick="ambilpilih('<?=$row[0]?>','<?=$kelas?>')" style="cursor:pointer"><?=$cnt ?></td>
    <td align="center" height="25" <?=$bg?> onClick="ambilpilih('<?=$row[0]?>','<?=$kelas?>')" style="cursor:pointer"><?=$row[0] ?></td>
