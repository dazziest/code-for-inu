...
if(!$user_exists) {
	...
	if ($username=="landlord"){
    	$query = "UPDATE jbsuser.landlord SET lastlogin='".date("Y-m-d H:I:s")."' WHERE password='".md5($password)."'";
    } else {
		$query = "UPDATE jbsuser.hakakses SET lastlogin='".date("Y-m-d H:I:s")."' WHERE login='$username' AND modul = 'SIMAKA'";
	}
