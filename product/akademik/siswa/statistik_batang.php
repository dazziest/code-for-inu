...
if ($iddasar=="12"){
	...
	$query3 = "SELECT COUNT(*) As Jum FROM jbsakad.siswa s, jbsakad.angkatan a WHERE $kondisi AND s.aktif = '1' AND s.penghasilanayah+s.penghasilanibu>=2500000 AND s.penghasilanayah+s.penghasilanibu<5000000";
	...
	if($sum == 0) {
	  echo "<table width='100%' height='100%'><tr><td align='center' valign='middle'>
		...
		*/
		$b1plot->SetFillColor($color[4]);
		$b2plot->SetFillColor($color[3]);
		$b3plot->SetFillColor($color[2]);
		$b4plot->SetFillColor($color[1]);
		$b5plot->SetFillColor($color[0]);
		//SetFillColor($color[$i]);
	...
	if ($iddasar=="1") {
		...
		$query1 = "SELECT COUNT(s.replid) As Jum, s.agama, g.replid FROM jbsakad.siswa s, jbsakad.angkatan a, jbsumum.agama g WHERE $kondisi AND s.aktif = 1 AND g.agama = s.agama GROUP BY s.agama ORDER BY g.agama";
	} if ($iddasar=="2") {
		$xaxis="Asal Sekolah";
		$yaxis="Jumlah Siswa";
	...
	if($num == 0) {
	  echo "<table width='100%' height='100%'><tr><td align='center' valign='middle'>
		...
		if ($iddasar == "2" || $iddasar=="6" ) { 
			...
			//$plot->SetFillColor(array_reverse($color));
