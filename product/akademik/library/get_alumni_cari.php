...
if ((strlen($nama) > 0) && (strlen($nip) > 0))
	...
	$sql = "SELECT s.nis, s.nama, k.kelas FROM jbsakad.siswa s,jbsakad.kelas k,jbsakad.tingkat ti,jbsakad.tahunajaran ta WHERE k.replid=s.idkelas AND s.statusmutasi=0 AND k.idtahunajaran=ta.replid AND k.idtingkat=ti.replid AND ta.departemen='$departemen' AND ti.departemen='$departemen'  AND s.alumni=0 ORDER BY s.nama"; 
$result = QueryDb($sql);
$cnt = 1;
