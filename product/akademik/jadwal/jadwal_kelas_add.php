...
if (isset($_REQUEST['Simpan'])) {		
	...
	//$sql3 = "SELECT * FROM jadwal WHERE idkelas = '$kelas' AND hari = $hari AND jam1 < '$jm2'";
	//$sql4 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND hari = $hari AND nipguru = '$nipguru' AND jam2 > '$jm1'";
	
	$sql3 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND idkelas = '$kelas' AND hari = $hari AND jamke BETWEEN '$jam' AND '$jam2'";
	$result3 = QueryDb($sql3);
	
	$sql4 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND hari = $hari AND nipguru = '$nipguru' AND jamke BETWEEN '$jam' AND '$jam2'";
	...
	if (mysql_num_rows($result3) > 0) {
		...
		$sql = "INSERT INTO jadwal SET idkelas=$kelas, nipguru='$nipguru', idpelajaran = $pelajaran, departemen = '$departemen', infojadwal = $info, hari = $hari, jamke = $jam, njam = $jum, sifat = 1, status = $status, keterangan='$keterangan', jam1 = '$jm1', jam2 = '$jm2', idjam1 = $rep1, idjam2 = $rep2";
		$result = QueryDb($sql);
		CloseDb();
...
<title>Akademik [Tambah Jadwal Kelas]</title>
...
<tr>
	<td align="left"><strong>Pelajaran</strong></td>
 	<td><div id ="InfoPelajaran">
      	<select name="pelajaran" id="pelajaran" onKeyPress="return focusNext('nip', event)" onFocus="panggil('pelajaran')" >
   	<?	OpenDb();
