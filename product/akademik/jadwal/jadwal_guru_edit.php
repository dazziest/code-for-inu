...
OpenDb();
$sql = "SELECT *, j.keterangan AS keterangan, idtahunajaran, idtingkat, t.tahunajaran, p.nama AS guru FROM jadwal j, kelas k, tahunajaran t, jbssdm.pegawai p WHERE j.replid = $replid AND j.idkelas = k.replid AND k.idtahunajaran = t.replid AND p.nip = j.nipguru";
$result = QueryDb($sql);
CloseDb();
...
if (isset($_REQUEST['Simpan'])) {		
	...
	//$sql3 = "SELECT * FROM jadwal WHERE nipguru = '$nip' AND hari = $hari AND jam1 < '$jm2' AND replid <> $replid ";
	$sql3 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND hari = $hari AND nipguru = '$nipguru' AND jamke BETWEEN '$jam' AND '$jam2' AND replid <> $replid";	
	$result3 = QueryDb($sql3);
		
	///$sql4 = "SELECT * FROM jadwal WHERE idkelas = '$kelas' AND hari = $hari AND jam2 > '$jm1' AND replid <> $replid";
	$sql4 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND idkelas = '$kelas' AND hari = $hari AND jamke BETWEEN '$jam' AND '$jam2' AND replid <> $replid";
	...
	if (mysql_num_rows($result3) > 0) {
		...
		CloseDb();		
		$ERROR_MSG = "Ada jadwal di kelas ini yang bentrok!";
	} else {
		$sql = "UPDATE jadwal SET idkelas=$kelas, idpelajaran = $pelajaran, departemen = '$departemen', infojadwal = $info, hari = $hari, jamke = $jam, njam = $jum, sifat = 1, status = $status, keterangan='$keterangan', jam1 = '$jm1', jam2 = '$jm2', idjam1 = $rep1, idjam2 = $rep2 WHERE replid = $replid";
		$result = QueryDb($sql);
		CloseDb();
...
<title>Akademik [Ubah Jadwal Guru]</title>
...
function change_tingkat() {
	...
	document.location.href = "jadwal_guru_edit.php?departemen="+departemen+"&pelajaran="+pelajaran+"&tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&hari="+hari+"&jam="+jam+"&jam2="+jam2+"&status="+status+"&keterangan="+keterangan+"&maxJam="+maxJam+"&nip="+nip+"&info="+info;		
}
