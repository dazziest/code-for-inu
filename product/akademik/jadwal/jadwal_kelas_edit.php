...
OpenDb();
$sql = "SELECT *, j.keterangan AS ket, k.kelas, p.nama, t.replid AS tahun, t.tahunajaran FROM jadwal j, jbssdm.pegawai p, jbsakad.tahunajaran t, kelas k WHERE j.replid = $replid AND j.nipguru = p.nip AND k.replid = j.idkelas AND k.idtahunajaran = t.replid";
$result = QueryDb($sql);
CloseDb();
...
if (isset($_REQUEST['Simpan'])) {		
	...
	$sql3 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND idkelas = '$kelas' AND hari = $hari AND jamke BETWEEN '$jam' AND '$jam2' AND replid <> $replid";
	$result3 = QueryDb($sql3);
	
	$sql4 = "SELECT * FROM jadwal WHERE departemen = '$departemen' AND infojadwal = $info AND hari = $hari AND nipguru = '$nipguru' AND jamke BETWEEN '$jam' AND '$jam2' AND replid <> $replid";		
...
<title>Akademik [Ubah Jadwal Kelas]</title>
...
<tr>
	<td align="left"><strong>Pelajaran</strong></td>
 	<td><div id ="InfoPelajaran">
      	<select name="pelajaran" id="pelajaran" onKeyPress="return focusNext('jam2', event)" onFocus="panggil('pelajaran')">
   	<?	OpenDb();
...
<tr>
    ...
    <td><strong>
    	<input type="text" name="nip" id="nip" size="10" class="disabled" value="<?=$nipguru ?>" readonly onKeyPress="pegawai();" onFocus="panggil('nip')" onClick="pegawai()"/>
        <input type="hidden" name="nipguru" id="nipguru" value="<?=$nipguru ?>"/>
    	<input type="text" name="nama" id="nama" size="20" class="disabled" value="<?=$nama ?>" readonly onClick="pegawai()" onFocus="panggil('nama')" />
