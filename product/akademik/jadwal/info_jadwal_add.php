...
if (isset($_REQUEST['Simpan'])) {
	...
	if (mysql_num_rows($result_simpan_cek) > 0) {
		...
		$sql_simpan="INSERT INTO jbsakad.infojadwal SET deskripsi='$deskripsi', idtahunajaran = $tahunajaran";  
		$result_simpan=QueryDb($sql_simpan);
		
		if ($result_simpan){
			$sql1 = "SELECT LAST_INSERT_ID(replid) FROM jbsakad.infojadwal ORDER BY replid DESC LIMIT 1";
			$result1 = QueryDb($sql1);
			$row1 = mysql_fetch_row($result1); 
			$lastid = $row1[0];		
			$sql = "UPDATE infojadwal SET aktif=1 WHERE replid=$lastid";
			QueryDb($sql);
			$sql = "UPDATE infojadwal SET aktif=0 WHERE replid<>$lastid";
			QueryDb($sql);
			?>
...
<html>
<head>
<title>Akademik [Tambah Info Jadwal]</title>
